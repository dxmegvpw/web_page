...
\begin{frame}{アノマリー}
  ...
  カイラル変換$\psi\rightarrow e^{i\gamma^{5}\alpha(x)}\psi$に対するカレントの方程式は
  \begin{equation}
    \partial_{\mu}
...
\begin{frame}{\thesubsection. \subsecname}
  ...
  \begin{equation}
    \partial
    \ev*{\bar{\psi}(x)\gamma^{\mu}\gamma^{5}\psi(x)}
    =
    2im\ev*{\bar{\psi}(x)\gamma^{5}\psi(x)}
    +
    \textcolor{Green}{ 
      2i\left( \frac{e^2}{32\pi^2} \right)\varepsilon^{\mu\nu\rho\sigma}F_{\mu\nu}F_{\rho\sigma}
    }
    \nonumber
  \end{equation}
  を計算する。
